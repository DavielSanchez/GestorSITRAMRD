...
function MisIncidenciasProceso() {
    ...
    const [pendientes, setPendientes] = useState([]);
      ...
      const fetchIncidencias = async () => {
         try {
           const response = await fetch(`${API_LINK}/incidencia/proceso/${userId}`);
  ...
  return (
    ...
    <div className="bg-[#f1f1ff] shadow-md rounded-lg p-4 flex flex-row items-center gap-3">
      ...
      <span className="text-[#6a62dc] text-xl font-bold">
        Incidencias en proceso ({pendientes.cantidad || 0})
      </span>
    </div>
    </>
  )
...
export default MisIncidenciasProceso