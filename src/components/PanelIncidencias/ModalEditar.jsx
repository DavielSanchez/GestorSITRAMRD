  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_LINK}/incidencia/estado/${incidencia._id}`,
        {
          method: "PUT",
...
"
    >
      ...
      <div className="relative bg-white rounded p-6 w-80 shadow-lg">
        <h3 className="text-lg font-bold mb-4 text-[#6a62dc]">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <button
              type="button"
              onClick={onClose}
              className="bg-[#FF5353] text-gray-800 px-4 py-2 rounded hover:bg-gray-300 transition"
            >
              Cancelar
            </button>
          </div>
