...
export function ChoferesInactivo() {
  ...
  const FetchData = async () => {
    try {
      ...
      const Usuarios = data.filter((user) => user.estadoUsuario === 'inactivo' && user.userRol === 'Conductor').length;
      SetUser(Usuarios);
    } catch (error) {
  ...
  return (
    <div className={`bg-[#f1f1ff] text-[#6a62dc] shadow-md rounded-lg p-4 flex flex-row items-center gap-3`}>
      <DirectionsBusIcon />
      <div className={`text-[#6a62dc] text-xl font-bold`}>Conductores Inactivos: {User}</div>
    </div>
  );
}
