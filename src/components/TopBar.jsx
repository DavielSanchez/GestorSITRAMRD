...
export default function BarraSuperior({ title, theme }) {
  ...
  return (
    <header
      ...
      {/* Título en morado */}
      <div className="text-[#6a62dc] text-[40px] font-semibold font-['Inter']">{title}</div>
      ...
      <div className="flex items-center gap-4">
        ...
        {/* Menú del usuario */}
        <UserMenu theme={theme} primaryColors={primaryColors} primaryHover={primaryHover} />
      </div>
    </header>
  );
}
      ...
      {showMessages && (
        <div
          ...
          <div className="flex justify-between items-center mb-4">
            <div className="text-[#6a62dc] text-base font-semibold">Mensajes</div>
            <button className="text-red-500 text-sm font-medium">Marcar todos como leídos</button>
          </div>
...
function NotificationItem() {
  return (
    <div className="flex items-start gap-2">
      ...
      <div className="leading-5">
        <div className="text-[#212121] text-sm font-medium">Se ha registrado una incidencia</div>
        <div className="text-[#9e9e9e] text-xs">Hace 5 minutos</div>
      </div>
    </div>
  );
  ...
  return (
    <div className={`relative flex items-center cursor-pointer p-2 rounded-full ${primaryHover}`}>
      <button
        ...
        <img src="https://placehold.co/45x45" alt="Avatar" className="w-full h-full object-cover" />
      </button>
      {showDropdown && (
...
function UserCard({ theme }) {
  return (
    <div className="w-[228px] h-[239px] bg-white rounded-2xl border border-[#6a62dc] p-4 z-2">
      <div className="flex flex-col gap-4">
        {/* Opción: Mi cuenta */}
